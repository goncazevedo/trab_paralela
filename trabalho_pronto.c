...
int *pProcInd;          // Número da primeira linha da matriz no processo
...
void PrintMatrix(double *matriz, int nLinhas, int nColunas){
    ...
    for (i = 0; i < nLinhas; i++){
        for (j = 0; j < nColunas; j++)
            printf("%7.4f ", matriz[i * nColunas + j]);
        printf("\n");
    }
}
...
void PrintSistema(double *matriz, double *vetorB, int nLinhas, int nColunas){
    ...
    for (i = 0; i < nLinhas; i++){
        for (j = 0; j < nColunas; j++)
            printf("%7.4fX%d ", matriz[i * nColunas + j], j);
        printf("= %7.4f", vetorB[i]);
        printf("\n");
    }
}
// Popula a matriz com zeros e uns
void DummyDataInitialization(double *matriz, double *vetorB, int tamanho){
    int i, j;
    for (i = 0; i < tamanho; i++){
        vetorB[i] = i + 1;
        for (j = 0; j < tamanho; j++){
...
void ProcessInitialization(double **matriz, double **vetorB,
                           double **vetorResultado, double **linhaDaMatriz, double **vetorBNoProcesso,
                           double **vetorResultadoNoProcesso, int *tamanho, int *linhasDaMatrizNoProcesso){
    int restRows; // Numero de linhas que não foram distribuídas
    int i;
    if (procRank == 0){
        do{
            printf("\nEnter the tamanho of the matrix and the vector: ");
            scanf("%d", tamanho);
    ...
    *linhaDaMatriz = malloc(sizeof(double) * (*linhasDaMatrizNoProcesso * (*tamanho)));
    *vetorBNoProcesso = malloc(sizeof(double)* (*linhasDaMatrizNoProcesso));
    *vetorResultadoNoProcesso = malloc(sizeof(double)* (*linhasDaMatrizNoProcesso));
...
void ResultCollection(double *vetorResultadoNoProcesso, double *vetorResultado){
    MPI_Gatherv(vetorResultadoNoProcesso, pProcNum[procRank], MPI_DOUBLE, vetorResultado,
...
void PrintResultVector(double *vetorResultado, int tamanho){
    int i;
    for (i = 0; i < tamanho; i++)
        printf("%7.4f ", vetorResultado[pParallelPivotPos[i]]);
}
// Eliminação de colunas paralela
void ParallelEliminateColumns(double *linhaDaMatriz, double *vetorBNoProcesso,
                              double *pPivotRow, int tamanho, int linhasDaMatrizNoProcesso, int iter){
    double multiplier;
    for (int i = 0; i < linhasDaMatrizNoProcesso; i++){
...
void ParallelGaussianElimination(double *linhaDaMatriz, double *vetorBNoProcesso,
                                 int tamanho, int linhasDaMatrizNoProcesso){
    ...
    for (int i = 0; i < tamanho; i++){
        ...
        for (int j = 0; j < linhasDaMatrizNoProcesso; j++){
            if ((pProcPivotIter[j] == -1) && (maxValue < fabs(linhaDaMatriz[j * tamanho + i]))){
...
void ParallelBackSubstitution(double *linhaDaMatriz, double *vetorBNoProcesso,
    ...
    for (int i = tamanho - 1; i >= 0; i--){
        ...
        if (procRank == iterProcRank){
            iterResult =
                vetorBNoProcesso[iterPivotPos] / linhaDaMatriz[iterPivotPos * tamanho + i];
...
void ParallelResultCalculation(double *linhaDaMatriz, double *vetorBNoProcesso,
                               double *vetorResultadoNoProcesso, int tamanho, int linhasDaMatrizNoProcesso){
    ParallelGaussianElimination(linhaDaMatriz, vetorBNoProcesso, tamanho, linhasDaMatrizNoProcesso);
    ParallelBackSubstitution(linhaDaMatriz, vetorBNoProcesso, vetorResultadoNoProcesso,
                             tamanho, linhasDaMatrizNoProcesso);
}
// Liberando a memória alocada
void ProcessTermination(double *matriz, double *vetorB, double *vetorResultado,
                        double *linhaDaMatriz, double *vetorBNoProcesso, double *vetorResultadoNoProcesso){
    if (procRank == 0){
        free(matriz);
        free(vetorB);
        free(vetorResultado);
 ...
 of the linear system matrix by the vector of unknowns */
    ...
    if (procRank == 0){
        ...
        for (int i = 0; i < tamanho; i++){
            ...
            for (int j = 0; j < tamanho; j++){
                pRightPartVector[i] +=
                    matriz[i * tamanho + j] * vetorResultado[pParallelPivotPos[j]];
            }
        }
        for (int i = 0; i < tamanho; i++){
            if (fabs(pRightPartVector[i] - vetorB[i]) > Accuracy) 
                equal = 1;
        }
        if (equal == 1)
...
void main(int argc, char *argv[]){
    ...
    if (procRank == 0)
        printf("Algoritmo paralelo para resolver sistemas lineares com eliminação de Gauss\n");
    // Alocação de memória e inicialização de dados
    ProcessInitialization(&matriz, &vetorB, &vetorResultado,
                          &linhaDaMatriz, &vetorBNoProcesso, &vetorResultadoNoProcesso, &tamanho, &linhasDaMatrizNoProcesso);
    inicio = MPI_Wtime();
    // Distribuição de dados nos processos
    DataDistribution(matriz, linhaDaMatriz, vetorB, vetorBNoProcesso, tamanho, linhasDaMatrizNoProcesso);
    // Cálculo em paralelo 
    ParallelResultCalculation(linhaDaMatriz, vetorBNoProcesso, vetorResultadoNoProcesso,
                            tamanho, linhasDaMatrizNoProcesso);
    // Printando os processos
    TestDistribution(matriz, vetorB, linhaDaMatriz, vetorBNoProcesso, tamanho, linhasDaMatrizNoProcesso);
    ...
    if (procRank == 0)
        printf("\n Tempo de execução: %f\n", duracao);
    // Liberando a memória alocada
    ProcessTermination(matriz, vetorB, vetorResultado, linhaDaMatriz, vetorBNoProcesso,
